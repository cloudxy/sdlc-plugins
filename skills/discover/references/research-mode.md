...
2. Select the relevant survey tasks using discover scope selection; execute §2 and, when positioning is requested, §2b. Use packet v2 with product_context, no SKILL.md paste. Record `host_spawn` on unknown type — **merge** into the existing mapping; never rewrite `host_spawn: {}`.
...
4. Only if every required survey gate exits 0, set only the executed and passed tracks to `done`; omitted tracks are `skipped` with reason. Leave `discovery.status: pending`. Return paths + short summary. Tell the user to run `/sdlc-discover` (discuss + falsify) or `/sdlc` after briefing freeze. Do not spawn pm.
