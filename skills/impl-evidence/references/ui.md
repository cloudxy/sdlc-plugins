...
| Task | Approach |
|---|---|
| **Implement a page/component** | Read story + tokens + edge-states → component tree → implement → 9-dim self-check → build verify |
...
4. Compare with the picked prototype screenshots: layout, hierarchy, the signature moment, state copy. Fix visible gaps now; record the rest.
...
| Direction | Content |
|---|---|
| **Input** | Ticket (journey slice + FR anchors) · spec §2 core value · picked prototype screenshots · `flows.md` · edge-states matrix · tokens / `design-system.md` · API contracts + examples · `tracking.md` |
