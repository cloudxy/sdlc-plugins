...
迁移与代码不是同时生效的。发布过程中必然存在一个窗口，**新旧两版代码同时在跑**（滚动发布、多副本、灰度、回滚）。一步到位的变更让这个窗口成为故障窗口：
...
```
expand（扩展）  新旧结构并存，只加不减 —— 可随时回滚
migrate（迁移） 回填数据 + 双写，切读 —— 可随时回滚
contract（收缩） 删掉旧结构 —— 确认无人使用后才做
```

三步分成**三个独立迁移文件**，跨越至少两次发布。写在一个文件里等于没做。
...
UPDATE users SET username = NULL;   -- 幂等，安全
```
...
`down` 能把数据找回来——这是 contract 步骤 `down` 的正确写法。做不到时（数据已不可重建）在迁移注释里显式声明「此步不可逆」，并要求交付前有备份。
...
-- Step 2 migrate：应用层加插入前校验，阻止新的重复产生
