...
- 精确金额可用 `DECIMAL(p, s)` 或明确单位的整数；说明币种、范围和舍入，避免二进制浮点作为精确账务权威
...
- 唯一键包含业务要求的作用域；租户内邮箱唯一应含 tenant_id，全局 ID/全局唯一事实不要机械添加
- 索引列顺序按实际访问模式；索引不能替代租户授权
...
**先查后插在并发下必漏**——两个请求同时查到「不存在」,同时插入。采用唯一约束/条件写/受控事务等正确并发协议。重复键不等于副作用已成功：区分处理中、失败与完成，并恢复去重记录和外部副作用之间的窗口。
